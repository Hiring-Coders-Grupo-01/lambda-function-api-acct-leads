...
exports.handler = async (event, context) => {
    ...
    switch (event.routeKey) {
      ...
      case "POST /leads/{uuid}/actions/convert":
        ...
        if (leadParaConversao.status == "Cliente") {
          throw new Error(`Lead de UUID: ${event.pathParameters.uuid} já convertido`);
        }
      ...
      default:
        throw new Error(`Rota não suportada: "${event.routeKey}`);
    }
  } catch (err) {
    statusCode = 400;
    body = err.message;
  ...
  async function findLeadByUuid(uuid) {
    ...
    if (!lead.Item) {
      throw new Error(`Não foi encontrado lead com UUID: ${uuid}`);
    }
